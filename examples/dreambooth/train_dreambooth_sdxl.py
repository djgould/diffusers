...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        for model in models:
            if isinstance(model, type(accelerator.unwrap_model(unet))):
                ...
                sub_dir = "text_encoder_one"
            elif isinstance(model, type(accelerator.unwrap_model(text_encoder_two))):
                sub_dir = "text_encoder_two"
            else:
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(model, type(accelerator.unwrap_model(text_encoder_one))):
                ...
                load_model = text_encoder_cls_one.from_pretrained(input_dir, subfolder="text_encoder_one")
                model.config = load_model.config
            elif isinstance(model, type(accelerator.unwrap_model(text_encoder_two))):
                load_model = text_encoder_cls_two.from_pretrained(input_dir, subfolder="text_encoder_two")
                model.config = load_model.config
    ...
    if accelerator.is_main_process:
        ...
        if text_encoder_one is not None and text_encoder_two is not None:
